...
class DatabaseManager:
    ...
    def get_table_data_with_join(self, table_name):
        """Получить данные из конкретной таблицы"""
        cursor = self.connection.cursor()
        expertColumns = self.get_columns_names(table_name)
        # expertGrntiColumns = self.get_columns_names("expert_grnti")
        cursor.execute(
            f'''
                SELECT 
                    e.id,
                    ...
                    eg.siscipline
                FROM expert e
                INNER JOIN expert_grnti eg ON e.id = eg.id 
                ORDER BY {expertColumns[0]};
            ''')
        data = cursor.fetchall()
        cursor.close()
        return data
    
    def get_columns_names_with_join(self):
        """Получить названия столбцов таблицы"""
        expertColumns = self.get_columns_names("expert")
        expertGrntiColumns = self.get_columns_names("expert_grnti")
        expertColumns.extend(expertGrntiColumns)
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def show_table_with_join(self, table_name):
        """Отображение содержимого таблицы"""
        try:
            self.current_table = table_name            

            data = self.db.get_table_data_with_join(table_name)
            columns = self.db.get_columns_names_with_join()
